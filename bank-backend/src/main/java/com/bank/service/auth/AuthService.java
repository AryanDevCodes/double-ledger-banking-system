...
import com.bank.service.notification.EmailService;
...
public class AuthService {
  ...
  public AuthResponseDTO login(LoginRequestDTO request) {
    ...
    boolean passwordProvided = request != null && StringUtils.hasText(request.getPassword());

    // Allow password-less first login (e.g. just-provisioned account); otherwise
    // authenticate.
    if (!(firstLogin && !passwordProvided && !hasPassword)) {
      authenticationManager.authenticate(
          new UsernamePasswordAuthenticationToken(request.getUsername(), request.getPassword()));
    }

    boolean passwordChangeRequired = firstLogin || !hasPassword;
  ...
  public ForgotPasswordResponseDTO requestPasswordReset(ForgotPasswordRequestDTO request) {
    ...
    // Always return the same generic response to avoid revealing which identifiers
    // exist.
    ForgotPasswordResponseDTO genericResponse = ForgotPasswordResponseDTO.builder()
    ...
    // Deliver the token out-of-band; do NOT include it in the API response.
  ...
  private void applyPersonalBankingMetrics(
    ...
    List<Account> ownAccounts = accountRepository.findByCustomerUserId(user.getId());
    BigDecimal totalBalance = ownAccounts.stream()
        .map(Account::getBalance)
        .filter(balance -> balance != null)
        .reduce(BigDecimal.ZERO, BigDecimal::add);

    long upiProfileCount = ownAccounts.stream()
    ...
    if (customer != null && StringUtils.hasText(customer.getEmail())) {
      ...
      ownAccounts.stream()
          ...
          .filter(StringUtils::hasText)
          .forEach(
              accountNumber -> transactionRepository
  ...
  private void applyManagedScopeMetrics(UserResponseDTO.UserResponseDTOBuilder builder, User user) {
    boolean canViewManagedScope = hasRole(user, Role.RoleName.ROLE_ADMIN)
    ...
    boolean canViewManagedTransactions = hasRole(user, Role.RoleName.ROLE_ADMIN)
        || hasRole(user, Role.RoleName.ROLE_MANAGER)
        || hasRole(user, Role.RoleName.ROLE_AUDITOR);
  ...
  private long countPendingKyc() {
    return customerRepository.findAll().stream()
        .filter(
            customer -> customer.getKycStatus() != Status.COMPLETED
