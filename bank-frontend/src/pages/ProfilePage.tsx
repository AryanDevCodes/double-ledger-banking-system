                ...
                <Card className="panel-luxe">
                  ...
                  <CardContent className="space-y-2">
                    {profileSignals.map((item) => (
                      <div key={item.label} className="flex items-start justify-between gap-3 rounded-lg border border-border/60 bg-background/55 px-3 py-2 text-sm">
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {accountsView.slice(0, 10).map((item) => (
                              <tr key={item.accountNumber} className="border-t border-border/60">
                    ...
                    {upiView.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {upiView.slice(0, 8).map((item) => (
                          <div key={`${item.upiId}-${item.id}`} className="rounded-lg border border-border/70 bg-muted/15 p-3">
                  ...
                  {transactionsView.length === 0 ? (
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      {transactionsView.slice(0, 8).map((item) => (
                        <div key={item.transactionId} className="rounded-lg border border-border/70 bg-muted/15 p-3">
                          <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                            <span>{formatDateTime(item.transactionDate)}</span>
                        ...
                        {topBanks.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {topBanks.map(([bankName, count]) => (
                              <div key={bankName} className="flex items-center justify-between rounded-lg border border-border/60 px-3 py-2">
                                <span>{bankName}</span>
