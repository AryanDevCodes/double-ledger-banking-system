...
public class QRCodeController {
  ...
  /**
   * Generate QR code for UPI payment Format:
  ...
  public ResponseEntity<byte[]> generateQRCode(
    ...
    try {
      ...
      BitMatrix bitMatrix = qrCodeWriter.encode(upiString.toString(), BarcodeFormat.QR_CODE, width, height, hints);
  ...
  public ResponseEntity<byte[]> generateAccountQR(
      ...
      @RequestParam(defaultValue = "300") int height) {
    try {
      String accountInfo = String.format("Account: %s\nBank: %s\nIFSC: %s", accountNumber, bankName, ifscCode);
