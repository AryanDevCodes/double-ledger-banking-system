...
public class UpiResolver {
  ...
  public Account resolveActiveAccount(String upiId) {
    ...
    if (linkedAccount == null) {
      throw new IllegalStateException("UPI profile '" + upiId + "' has no linked account");
    }
    if (linkedAccount.getStatus() != Status.ACTIVE) {
      throw new InvalidDataException(
          "Account linked to UPI '" + upiId + "' is not active (status="
              + linkedAccount.getStatus() + ")");
    }
  ...
  public Account resolveAndVerifyOwnership(String upiId, String username) {
     ...
     * boolean matches = customerName != null &&
     * customerName.equalsIgnoreCase(username)
     * || customerEmail != null
     ...
     * // log.warn("Using legacy UPI ownership fallback for username: {}",
     * username);
     * return account;
     */
  }
