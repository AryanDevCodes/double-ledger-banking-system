...
public class UpiResolver {
    ...
    public Account resolveAndVerifyOwnership(String upiId, String username) {
        ...
        if (username == null || username.isBlank()) {
            throw new AccessDeniedException("Authenticated username is required for ownership verification");
        }
        ...
        if (customer == null) {
            throw new IllegalStateException("Account has no associated customer: " + account.getAccountNumber());
        }
        ...
        if (customer.getUser() != null) {
            ...
            if (accountOwnerUsername == null || !accountOwnerUsername.equals(username)) {
                throw new AccessDeniedException(
                    "Authenticated user '" + username + "' does not own UPI ID '" + upiId + "'. " +
                    "UPI belongs to user: " + accountOwnerUsername
                );
            }
        ...
        // ── Legacy fallback
        // Consider removing this block after data migration
        throw new AccessDeniedException(
            "UPI ownership cannot be verified: Customer has no linked User account. " +
            "UPI ID: " + upiId + " | Contact support or migrate account linkage."
        );
        ...
        if (!matches) {
            throw new AccessDeniedException(
                "Authenticated user '" + username + "' does not own UPI ID '" + upiId + "'. " +
                "Possible owner: " + customerName + " (" + customerEmail + ")"
            );
        }
